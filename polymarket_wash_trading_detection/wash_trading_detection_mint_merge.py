...
def filter_self_trades(trades, save = True):
    ...
    if save:
        self_trades.to_csv(os.path.join(os.path.dirname(__file__),"./output/self_trades_mint_and_merge.csv"), index=False)
    return reg_trades, self_trades

def add_trader_hashes(trades, load_existing_hashes = True, save = True):
    ...
    if (global_trader_hashes.shape[0] == 0):
        ...
        global_trader_hashes.loc[len(global_trader_hashes)] = [exchange_address, str(0)]
    else:
        unique_addresses = sorted(pd.concat([trades["token_buyer"], trades["token_seller"]]).unique())
        additional_traders = list(set(unique_addresses) - set(global_trader_hashes["trader_address"]))
        if exchange_address in additional_traders:
            additional_traders.remove(exchange_address)
        ...
        if exchange_address not in global_trader_hashes["trader_address"].tolist():
            global_trader_hashes.loc[len(global_trader_hashes)] = [exchange_address, str(0)]
...
def detect_scc_for_tokens(trades, save=True):
    ...
    if save:
        scc_dt.to_csv(os.path.join(os.path.dirname(__file__),"./output/scc_mint_and_merge.csv"), index=False)
        scc_mapping = pd.DataFrame([
            {"hash": h, "trader_id": trader_id}
            for h, trader_ids in global_scc_traders_map.items()
            for trader_id in trader_ids
        ])
        scc_mapping.to_csv(os.path.join(os.path.dirname(__file__),"./output/scc_mapping_mint_and_merge.csv"), index=False)
    return scc_dt
...
def detect_and_label_wash_trades_for_scc_with_multiple_time_windows(trades, repeated_scc, window_sizes_in_seconds, window_start, margin):
    global global_scc_traders_map
    ...
    for window_size in window_sizes_in_seconds:  
        ...
        for scc_id in tqdm(repeated_scc["scc_hash"], desc="scc volume matching"): # rows of scc_dt
            ...
            # only process scc that does not involve exchange, otherwise will take huge amount of time
            if "0" in scc_traders:
                continue
            # print(scc_traders)
            ...
            trades.loc[trades["orderHash"].isin(scc_trades["orderHash"]), "is_wash_trade"] = False 
            ...
            for _, single_scc_trade in scc_trades_per_token_and_time_window.items():
                cond_1 = trades["transactionHash"].isin(single_scc_trade["transactionHash"]) & ~(trades["orderHash"].isin(single_scc_trade["orderHash"])) & trades["is_wash_trade"].isnull()
                cond_2 = trades["transactionHash"].isin(single_scc_trade["transactionHash"]) & ~(trades["orderHash"].isin(single_scc_trade["orderHash"])) & (trades["is_wash_trade"] == False)
                other_trades_in_txn = trades[cond_1 | cond_2]
                # scc_wash_trades = detect_and_label_wash_trades_using_volume_matching(other_trades_in_txn, single_scc_trade, margin)
                scc_wash_trades = detect_and_label_wash_trades_using_ilp_volume_matching_dynamic_bound(other_trades_in_txn, single_scc_trade, margin)
                if scc_id not in wash_trades.keys():
                    wash_trades[scc_id] = {}
                if str(window_size) not in wash_trades[scc_id].keys():
                    wash_trades[scc_id][str(window_size)] = []
                ...
                trades.loc[trades["orderHash"].isin(scc_wash_trades[scc_wash_trades["is_wash_trade"] == True]["orderHash"]), "is_wash_trade"] = True
    return trades, wash_trades

def detect_and_label_wash_trades_using_ilp_volume_matching_dynamic_bound(other_trades_in_txn, scc_trades, margin):
    trades = pd.concat([other_trades_in_txn, scc_trades])
...
def detect_and_label_wash_trades_using_volume_matching(other_trades_in_txn, scc_trades, margin = 0.1):
    trades = pd.concat([other_trades_in_txn, scc_trades])

    # verify exchange is involved, all trades are in the same market, and all tokens for that market is involved
    cond_1 = ("0" in trades["token_buyer_id"].tolist()) or ("0" in trades["token_seller_id"].tolist())
    cond_2 = len(trades["question"].unique().tolist()) == 1
    cond_3 = len(trades["token_id"].unique().tolist()) == 2
...
def get_wash_trades_summary(wash_trades, save=True):
    ...
    if save:
        summary_filtered.to_csv(os.path.join(os.path.dirname(__file__),"./output/wash_trades_summary_mint_and_merge.csv"), index=False)
...
# ------ run the algorithm ------
trades = pd.read_csv(os.path.join(os.path.dirname(__file__),"./data/polymarket_OrderFilled_preprocessed_mint_and_merge.csv"))
...
repeated_scc = get_repeated_scc(scc_dt, 3)
trades, wash_trades = detect_and_label_wash_trades_for_scc_with_multiple_time_windows(trades, repeated_scc, [600], None, 0.1)
get_wash_trades_summary(wash_trades)

# save to file
trades.to_csv(os.path.join(os.path.dirname(__file__), "./output/trades_labeled_mint_and_merge.csv"), index=False)
trades[trades["is_wash_trade"] == True].to_csv(os.path.join(os.path.dirname(__file__), "./output/wash_trades_labeled_mint_and_merge.csv"), index=False)